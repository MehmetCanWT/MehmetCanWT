...
export default function AdminPage() {
  ...
  useEffect(() => {
    // Sayfa yüklendiğinde daha önce giriş yapılıp yapılmadığını kontrol et
    const authStatus = localStorage.getItem("mehmetcanwt_admin_auth");
    if (authStatus === "true") {
      setIsAuth(true);
    }
    setLoading(false);
  ...
  if (loading) return null; // İlk yüklemede boş dön (flash engellemek için)
  ...
  return (
    <div className="min-h-screen p-8 max-w-4xl mx-auto space-y-8">
      <header className="flex justify-between items-center">
        <Link href="/" className="manga-panel flex items-center gap-2 font-black uppercase py-2 px-4 hover:bg-black hover:text-white transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <Link href="/admin/anime" className="manga-panel group hover:bg-black hover:text-white transition-all p-12 flex flex-col items-center justify-center space-y-4">
        ...
        <Link href="/admin/games" className="manga-panel group hover:bg-black hover:text-white transition-all p-12 flex flex-col items-center justify-center space-y-4">
